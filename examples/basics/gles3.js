...
var ASM_CONSTS = {
  245184: () => { Module.canvas.focus(); },  
 245209: ($0, $1, $2, $3, $4) => { var data = Module.HEAPU8.slice($0, $0 + $1 * $2 * 4); let context = document.getElementById("canvas").getContext("2d"); let image = context.getImageData(0, 0, $1, $2); image.data.set(data); context.putImageData(image, 0, $4 - $2); },  
 245444: () => { return window.devicePixelRatio || 1; },  
 245483: () => { return window.innerWidth || 0; },  
 245516: () => { return window.innerHeight || 0; },  
 245550: () => { window.addEventListener("keydown", (event) => { var code = stringToNewUTF8(event.code); Module._RGFW_handleKeyMods(event.getModifierState("CapsLock"), event.getModifierState("NumLock"), event.getModifierState("Control"), event.getModifierState("Alt"), event.getModifierState("Shift"), event.getModifierState("Meta"), event.getModifierState("ScrollLock")); var codepoint = event.key.charCodeAt(0); if(codepoint < 0x7f && event.key.length > 1) { codepoint = 0; } Module._RGFW_handleKeyEvent(code, codepoint, 1); _free(code); }, true); window.addEventListener("keyup", (event) => { var code = stringToNewUTF8(event.code); Module._RGFW_handleKeyMods(event.getModifierState("CapsLock"), event.getModifierState("NumLock"), event.getModifierState("Control"), event.getModifierState("Alt"), event.getModifierState("Shift"), event.getModifierState("Meta"), event.getModifierState("ScrollLock")); Module._RGFW_handleKeyEvent(code, 0, 0); _free(code); }, true); },  
 ...
 247409: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 247480: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 247537: () => { return window.mouseX || 0; },  
 247568: () => { return window.mouseY || 0; },  
 247599: ($0) => { var canvas = document.getElementById('canvas'); if ($0) { canvas.style.pointerEvents = 'none'; } else { canvas.style.pointerEvents = 'auto'; } },  
 247746: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 247799: ($0, $1) => { var ext = UTF8ToString($0, $1); var canvas = document.querySelector('canvas'); var gl = canvas.getContext('webgl') || canvas.getContext('experimental-webgl'); if (!gl) return 0; var supported = gl.getSupportedExtensions(); return supported && supported.includes(ext) ? 1 : 0; },  
 248079: () => { return window.innerWidth; },  
 248109: () => { return window.innerHeight; },  
 248140: () => { Module.requestFullscreen(false, true); },  
 248179: () => { Module.exitFullscreen(false, true); },  
 248215: ($0, $1) => { var element = document.getElementById("canvas"); if (element) element.style.opacity = $1; }
};
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 241568;
