...
var ASM_CONSTS = {
  85024: () => { Module.canvas.focus(); },  
 85049: () => { window.addEventListener("keydown", (event) => { Module._RGFW_handleKeyEvent(stringToNewUTF8(event.key), stringToNewUTF8(event.code), 1); }, true, ); },  
 85202: () => { window.addEventListener("keyup", (event) => { Module._RGFW_handleKeyEvent(stringToNewUTF8(event.key), stringToNewUTF8(event.code), 0); }, true, ); },  
 ...
 86372: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 86443: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 86500: () => { return window.mouseX || 0; },  
 86531: () => { return window.mouseY || 0; },  
 86562: ($0) => { var canvas = document.getElementById('canvas'); if ($0) { canvas.style.pointerEvents = 'none'; } else { canvas.style.pointerEvents = 'auto'; } },  
 86709: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 86762: () => { return window.innerWidth; },  
 86792: () => { return window.innerHeight; }
};
