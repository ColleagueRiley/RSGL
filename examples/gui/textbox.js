...
var ASM_CONSTS = {
  72408: () => { Module.canvas.focus(); },  
 72433: () => { window.addEventListener("keydown", (event) => { Module._RGFW_handleKeyEvent(stringToNewUTF8(event.key), stringToNewUTF8(event.code), 1); }, true, ); },  
 72586: () => { window.addEventListener("keyup", (event) => { Module._RGFW_handleKeyEvent(stringToNewUTF8(event.key), stringToNewUTF8(event.code), 0); }, true, ); },  
 ...
 73756: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 73827: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 73884: () => { return window.mouseX || 0; },  
 73915: () => { return window.mouseY || 0; },  
 73946: ($0) => { var canvas = document.getElementById('canvas'); if ($0) { canvas.style.pointerEvents = 'none'; } else { canvas.style.pointerEvents = 'auto'; } },  
 74093: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 74146: () => { return window.innerWidth; },  
 74176: () => { return window.innerHeight; }
};
